...
def load_model():
    """Load TimesFM 2.5 model (lazy loading)"""
    global model
    if model is None:
        try:
            ...
            # Load TimesFM 2.5 from Hugging Face
...
class ForecastRequest(BaseModel):
    data: List[float] = Field(..., description="Input time series data (context)", min_length=2)
...
class ForecastResponse(BaseModel):
    ...
    quantile_forecast: Optional[List[List[float]]] = Field(None, description="Quantile forecasts (mean + 10th-90th percentiles)")
